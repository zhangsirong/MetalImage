...
- (void)setupContentWithCVBuffer:(CVBufferRef)buffer {
    if (CVPixelBufferGetWidth(buffer) > [[self class] maximumTextureSizeForCurrentDevice] || CVPixelBufferGetHeight(buffer) > [[self class] maximumTextureSizeForCurrentDevice]) {
        CGSize scaledSize = [self scaleSizeBasingOnMaxTextureSize:CGSizeMake(CVPixelBufferGetWidth(buffer), CVPixelBufferGetHeight(buffer))];
        
        _size = scaledSize;
        [self udpateResizedMTLTextureWithBuffer:buffer newSize:scaledSize];
    } else {
        _size = CGSizeMake(CVPixelBufferGetWidth(buffer), CVPixelBufferGetHeight(buffer));
        [self updateMTLTextureWithCVBuffer:buffer size:_size];
    }
}
...
- (void)updateMTLTextureWithCVBuffer:(CVBufferRef)buffer size:(CGSize)bufferSize {
    ...
    CVReturn error = CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                               [MIContext sharedContext].CVMetalTextureCache,
                                                               buffer,
                                                               NULL,
